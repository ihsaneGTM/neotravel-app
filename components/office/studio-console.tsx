  ...
  if (!configured) {
    return (
      <div className="nt-card mx-auto max-w-xl p-8 text-center">
        ...
        <p className="mt-2 text-sm text-slate-500">Ajoute ces variables (.env.local + Vercel) puis redéploie :</p>
        <ul className="mt-3 inline-flex flex-col gap-1 text-left">{missing.map((m) => <li key={m} className="font-mono text-sm text-slate-700">• {m}</li>)}</ul>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div className="nt-card p-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {EXAMPLES.map((ex) => <button key={ex} onClick={() => setPrompt(ex)} disabled={stage === "proposing"} className="rounded-full border border-[var(--line)] bg-slate-50 px-3 py-1 text-xs text-slate-500 transition hover:bg-slate-100">{ex}</button>)}
        </div>
      ...
      {stage !== "idle" && (
        <div className="nt-card p-5">
          <div className="mb-1 flex items-center justify-between">
            <p className="text-sm font-semibold text-slate-800">Progression</p>
            {inflight && <span className="flex items-center gap-1.5 font-mono text-xs text-slate-500"><Loader2 className="h-3.5 w-3.5 animate-spin" /> {elapsed}</span>}
          </div>
              ...
              return (
                <li key={i} className="flex items-center gap-3">
                  <span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-semibold ${
                    s === "done" ? "bg-emerald-100 text-emerald-700" : s === "active" ? "bg-indigo-600 text-white" : s === "error" ? "bg-rose-100 text-rose-600" : "bg-slate-100 text-slate-400"
                  }`}>
                  ...
                  <span className={`text-sm ${s === "active" ? "font-medium text-slate-800" : s === "pending" ? "text-slate-400" : "text-slate-600"}`}>{st.label}</span>
                  {s === "active" && <span className="ml-auto font-mono text-xs text-slate-400">{elapsed}</span>}
                </li>
              );
            })}
          </ol>
      ...
      {proposal && (stage === "proposed" || stage === "creating") && (
        <div className="nt-card p-5">
          <div className="flex items-start justify-between gap-3">
            <div><h3 className="text-base font-semibold text-slate-900">{proposal.summary}</h3><p className="mt-1 text-sm text-slate-500">{proposal.explanation}</p></div>
      ...
      {(stage === "preview" || stage === "merging" || stage === "merged") && (
        <div className="nt-card p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2"><DeployBadge state={stage === "merged" ? "ready" : deploy.state} />{branch && <span className="font-mono text-xs text-slate-400">branche {branch}</span>}</div>
            <div className="flex items-center gap-2"><span className="text-xs text-slate-400">Page :</span>
          ...
          {stage === "merged" ? (
            <div className="mt-4 rounded-xl bg-emerald-50 p-4 text-center">
              ...
              <p className="text-xs text-emerald-700/80">La production se redéploie (~1 min). Rafraîchis ensuite pour voir le résultat.</p>
                ...
                {deploy.state === "ready" && deploy.url ? <Frame label="Après (preview)" src={deploy.url + route} accent /> : (
                  <div className="grid h-[460px] place-items-center rounded-xl border border-dashed border-[var(--line-2)] bg-slate-50 text-center">
                    <div>
                      ...
                      <p className="font-mono text-xs text-slate-400">{deploy.state === "error" ? "Le code proposé ne compile pas — merge bloqué." : elapsed}</p>
                    </div>
                  </div>
                )}
              </div>
...
function Frame({ label, src, accent }: { label: string; src: string; accent?: boolean }) {
  return (
    <div className={`overflow-hidden rounded-xl border ${accent ? "border-indigo-300 ring-1 ring-indigo-200" : "border-[var(--line)]"}`}>
      <div className="flex items-center justify-between bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-500"><span>{label}</span><a href={src} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-slate-600"><ExternalLink className="h-3.5 w-3.5" /></a></div>
...
function DeployBadge({ state }: { state: DeployState }) {
  const map = { pending: { t: "En attente", c: "bg-slate-100 text-slate-500", spin: true }, building: { t: "Build en cours", c: "bg-amber-50 text-amber-600", spin: true }, ready: { t: "Preview prête", c: "bg-emerald-50 text-emerald-700", spin: false }, error: { t: "Build échoué", c: "bg-rose-50 text-rose-600", spin: false } }[state];
